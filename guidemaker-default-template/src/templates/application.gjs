...
import '../../static/guidemaker-ember-template.css';

export default RouteTemplate(<template>
{{! template-lint-disable link-rel-noopener no-curly-component-invocation no-redundant-role require-valid-alt-text }}
<HeadLayout />

<header class="header">
  <nav role="navigation" aria-label="main" class="container">
    <a href={{rootUrl "/"}} class="logo">
      {{#if @controller.guidemaker.logo}}
        <img src={{@controller.guidemaker.logo}}>
      {{else}}
        <span>
          {{@controller.guidemaker.title}}
        </span>
      {{/if}}
    </a>
  ...
  <div class="footer responsive">
    <div class="container space-between">
      <div id="ember106" class="footer-info flex-1">
        {{if @controller.guidemaker.copyright @controller.guidemaker.copyright @controller.guidemaker.title}} &copy; {{@controller.currentYear}}
      </div>
      <div id="ember114" class="footer-statement footer-social">
          {{#if @controller.guidemaker.social.twitter}}
            <a href="http://twitter.com/{{@controller.guidemaker.social.twitter}}" target="_blank" title="Twitter">
          ...
          {{/if}}
          {{#if @controller.guidemaker.social.github}}
            <a href="https://github.com/{{@controller.guidemaker.social.github}}" target="_blank" title="GitHub">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 1000">
          ...
          {{/if}}
          {{#if @controller.guidemaker.social.discordLink}}
            <a href={{@controller.guidemaker.social.discordLink}} title="Discord">
